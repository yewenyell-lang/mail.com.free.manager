# Sorting Desk · mail.com 分拣台

面向 mail.com 免费邮箱的批量管理工作台：导入账号、登录、取件、验证码提取、单封撰写/回复/转发。

服务端**零持久化**。账号、token、邮件列表和正文只存在浏览器 IndexedDB（`sorting-desk-2`）。刷新页面不会丢账号；换浏览器或清站点数据需要重新导入。

## 功能

- 导入 `email:password` 或 `email----password`（密码含 `:` 时必须用 `----`）
- 账号勾选后批量登录 / 取件 / 导出 / 删除；右键复制邮箱、复制凭证、强制重登
- 已登录账号默认跳过完整 OAuth，行内刷新按钮只拉信
- 收件 / 已发送 / 回收站 / 垃圾箱切换；列表标收发方向
- 打开收件自动已读；勾选后才出现已读/星标/回收站等操作
- 验证码从主题/正文提取，一键复制
- TinyMCE 富文本撰写（字体、颜色、表格、插图、附件）；回复/转发引用原信
- 正文内嵌图转为 CID 附件发送；详情页可下载附件并还原 cid 图片
- 列表与正文走本机缓存，点「刷新」才向 mail.com 拉新
...
- mail.com 对并发敏感：批量登录/取件默认同时 3 个账号，单账号拉文件夹同时 5 个，没有额外限速
